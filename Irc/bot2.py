...
if __name__ == '__main__':
    bot = IrcBot('irc.freenode.org', 6667, 'NoahSucksBot', 'Problem', 'Peehead', 'Nob')

    bot.connect()
    bot.connect_to_channel('##dme')

    bot.register_command(lambda *args, **kwargs: 'Hi ' + kwargs['user'], 'hi')
    bot.register_command(bot.move_to_channel, 'move')
    bot.register_command(bot.list_users, 'list')
    bot.register_command(bot.quit, 'quit')
